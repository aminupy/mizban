...
def create_gradient_title(parent, bold_text, regular_text, font_name, font_size, start_color, end_color):
    """
    ...
    if num_chars == 0: return title_frame
    ...
    for i, char in enumerate(full_text):
        ...
        char_label = tk.Label(title_frame, text=char, font=current_font, fg=hex_color, bg=parent.cget("bg"))
...
class RoundedFrame(tk.Canvas):
    ...
    """
    def __init__(self, parent, radius=20, padding=10, bg_color="#f0f0f0", fg_color="white", **kwargs):
        ...
        self.window_id = self.create_window(padding, padding, window=self.inner_frame, anchor="nw")
    ...
    def _resize_and_redraw(self, event):
        ...
        self.itemconfig(self.window_id, width=canvas_width - 2*self.padding, height=canvas_height - 2*self.padding)
    ...
    def _draw_perfect_rounded_rect(self, width, height):
        ...
        if radius < 1:
            self.create_rectangle(0, 0, width, height, fill=self.fg_color, outline="", tags="shape")
            return

        # Draw the four corner arcs
        self.create_arc(0, 0, 2*radius, 2*radius, start=90, extent=90, fill=self.fg_color, outline="", tags="shape")
        self.create_arc(width - 2*radius, 0, width, 2*radius, start=0, extent=90, fill=self.fg_color, outline="", tags="shape")
        self.create_arc(0, height - 2*radius, 2*radius, height, start=180, extent=90, fill=self.fg_color, outline="", tags="shape")
        self.create_arc(width - 2*radius, height - 2*radius, width, height, start=270, extent=90, fill=self.fg_color, outline="", tags="shape")

        # Draw the four connecting rectangles (sides)
        self.create_rectangle(radius, 0, width - radius, height, fill=self.fg_color, outline="", tags="shape")
        self.create_rectangle(0, radius, width, height - radius, fill=self.fg_color, outline="", tags="shape")
